...
function ItemDAO(database) {
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         *
         */
        var queryCursor = (category == "All") ? {} : {"category": category};
        var cursor = this.db.collection("item").find(queryCursor);
    ...
    this.getNumItems = function(category, callback) {
         ...
         */
         ...
         var queryDoc = (category == "All") ? {} : {category: category};
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        var queryCursor = (query.trim() == "") ? {} : {$text: {$search: query}};
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        var searchQuery = (query.trim() == "") ? {} : {$text: {$search : query}};
        this.db.collection("item").find(searchQuery).count(function (err, res) {
          assert.equal(null, err);
          callback(res);
        })
    }
    ...
    this.getItem = function(itemId, callback) {
        ...
        this.db.collection("item").find({_id: itemId}).toArray(function (err, res){
          assert.equal(null, err);
          callback(res[0]);
        })
        /*
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        this.db.collection("item").updateOne(
          ...
          function (err, doc) {
            assert.equal(null, err);
            // Callback function does not actually use the return value.
            callback(doc);
          }
        )
