...
function CartDAO(database) {
    ...
    this.itemInCart = function(userId, itemId, callback) {
        ...
        this.db.collection("cart").find(query, projection).next(function (err, res) {
          var temp = (res == null) ? null : res.items[0];
          callback(temp);
        });
    ...
    this.updateQuantity = function(userId, itemId, quantity, callback) {
        ...
        */
        ...
        if (quantity == 0) {
          ...
          filter = { userId: userId, items: { $elemMatch: { _id: itemId } } };
          updateQuery = { $set: { "items.$.quantity" : quantity } };
        }

        this.db.collection("cart").findOneAndUpdate(filter, updateQuery,
          { returnOriginal: false },
          function (err, res) {
            assert.equal(null, err);
            callback(res.value);
